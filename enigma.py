...
class Reflector(): 
    ...
    def __init__(self):
        ...
        self.reflector=["AEPBLCGQKDJXUWRY ZOIVSNÑTMHF","SKIWYQ CEÑZRTBXLGJNPFAODUHMV"]
...
class Rotor():
    ...
    def conexion_decodifica(self,indice,rotor):
        ...
        return conector
    def salto(self,lista):#incorporo salto
        lista[0]=lista[0][1:]+lista[0][:1]
        lista[1]=lista[1][1:]+lista[1][:1]
        return lista
...
def start2(mensaje):
    global s
    t=[]
    for i in mensaje:#poner una condicion si lo escrito no esta en mayusculas ponerlas. 
        indice=e.abecedario.index(i) 
        indice=r.conexion(indice,e.rotor1)
        indice=r.conexion(indice,e.rotor2)    
        indice=r.conexion(indice,e.rotor3)
